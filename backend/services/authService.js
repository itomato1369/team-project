...
const googleCallback = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            console.error('[AuthService] Step 1 Failed: Error exchanging token with Google.');
            throw error;
        }
        ...
        try {
            ...
            console.error('[AuthService] Step 2 Failed: Error fetching user info from Google.');
            throw error;
        }
        ...
        try {
            ...
            console.error('[AuthService] Step 3 Failed: Error finding user by email.');
            throw error;
        }
        ...
        if (!users || users.length === 0) {
            isNewUser = true;
            console.log(`[AuthService] New Google user: ${email}. Creating new user.`);
            try {
                await db.query("createSocialUser", [email, name, email, googleId]);
                users = await db.query("findUserByEmail", [email]);
            } catch (error) {
                console.error('[AuthService] Step 4 Failed: Error creating social user.');
                throw error;
            }
        }
        ...
        if (needsAdditionalInfo) {
            ...
            // 추가 정보 입력 페이지로 리디렉션
            return res.redirect(`http://localhost:5173/auth/additional-info?signupToken=${signupToken}`);
        }

        // 6. 추가 정보가 필요 없는 기존 사용자 -> 바로 로그인
        console.log(`[AuthService] Existing Google user ${user.user_id} logged in.`);
        const { accessToken, refreshToken } = generateTokens(user);
        res.redirect(`http://localhost:5173/login-success?token=${accessToken}&refreshToken=${refreshToken}`);

    } catch (error) {
        console.error('[AuthService] Google callback error:', error.response ? error.response.data : error.message);
...
const completeSocialSignup = async (req, res) => {
    ...
    try {
        ...
        // 5. 토큰 및 사용자 정보 반환
        res.status(200).json({
            user: { id: user.user_id, name: user.user_name, email: user.email, role: user.user_role },
            accessToken,
            refreshToken
        });
