...
async function fetchLogDetail(consultNo) {
  try {
    ...
    if (form.ward_no) {
      await fetchSurveyList(form.ward_no);
    }
  } catch (err) {
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 md:p-10">
        <form @submit.prevent="handleSubmit" class="space-y-8">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8">
            <!-- Form Fields (동일 구조, isViewMode 바인딩 유지) -->
            <div class="space-y-2">
            ...
            <div class="space-y-2">
              ...
              <select
                ...
                :disabled="isViewMode"
                :class="isViewMode ? 'bg-gray-100 cursor-not-allowed' : 'bg-white'"
